...
def train_bilstm_model(x_train, x_test, y_train, y_test, model_filename='peptide_bilstm.keras', plot_filename='training_lost.png', epochs=50, batch_size=2):
...
def main():
    # Load data from CSV
    x_train = pd.read_csv("AVPdb_x_train_kmers.csv",index_col=0)
    y_train = pd.read_csv("AVPdb_y_train.csv",index_col=0)
    x_test = pd.read_csv("AVPdb_x_test_kmers.csv",index_col=0)
    y_test = pd.read_csv("AVPdb_y_test.csv",index_col=0)
